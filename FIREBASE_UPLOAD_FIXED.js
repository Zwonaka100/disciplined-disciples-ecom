// PASTE THIS INTO BROWSER CONSOLE ON SHOP PAGE TO UPLOAD PRODUCTS TO FIREBASE
// This fixes the JSON structure error from previous version
...
// Upload all products with retry logic
...
prods.forEach((p, idx) => {
  ...
  setTimeout(() => {
    console.log(`Uploading ${idx + 1}/${prods.length}: ${p.name}...`);
    console.log('  Product structure:', JSON.stringify({
      id: p.id,
      name: p.name,
      options: {
        colors: Object.keys(p.options?.colors || {}),
        sizes: p.options?.sizes?.length || 0
      }
    }));
    
    window.db.collection('products').doc(p.id).set(p, { merge: false })
      .then(() => {
        uploadCount++;
        console.log(`✓ [${uploadCount}] ${p.name} uploaded successfully`);
        if (uploadCount + errorCount === prods.length) {
          console.log(`\n✅ Upload complete: ${uploadCount}/${prods.length} products uploaded`);
          console.log('Refreshing products in 2 seconds...');
          setTimeout(() => {
            if (typeof window.fetchProductsFromFirestore === 'function') {
              window.fetchProductsFromFirestore().then(count => {
                console.log(`📦 Total products in system: ${count}`);
      ...
      .catch(e => {
        errorCount++;
        console.error(`✗ [${errorCount}] ${p.id} FAILED:`, e.message);
        if (uploadCount + errorCount === prods.length) {
          console.log(`\n⚠️ Upload complete with errors: ${uploadCount} uploaded, ${errorCount} failed`);
        }
      });
  }, idx * 500);
});
